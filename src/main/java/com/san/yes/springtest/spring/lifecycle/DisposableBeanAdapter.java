...
/**
 * 实现DisposableBean和Runnable接口的适配器，用于对给定bean实例执行各种销毁步骤
 * 1. DestructionAwareBeanPostProcessors
 * 2. bean自身实现了DisposableBean
 * 3. bean-definition只自定义的destroy-method
 */
...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	public void destroy() {
		if (!CollectionUtils.isEmpty(this.beanPostProcessors)) {
			// 执行销毁后置处理器的销毁前回调方法 @PreDestroy即在这里执行
