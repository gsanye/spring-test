...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * 销毁Bean实例：name->instance，实际为:DisposableBeanAdapter适配器
	 * */
	private final Map<String, DisposableBean> disposableBeans = new LinkedHashMap<>();

	/**
	 * Map between containing bean names: bean name to Set of bean names that the bean contains.
	 *包含bean name的映射：bean name->包含该bean的所有bean name集合
	 * */
	private final Map<String, Set<String>> containedBeanMap = new ConcurrentHashMap<>(16);

	/**
	 * Map between dependent bean names: bean name to Set of dependent bean names.
	 * 依赖bean name之间的映射：bean name->依赖该bean的bean name集合
	 *  */
	private final Map<String, Set<String>> dependentBeanMap = new ConcurrentHashMap<>(64);

	/**
	 * Map between depending bean names: bean name to Set of bean names for the bean's dependencies.
	 * 依赖bean name之间的映射：bean name-> 该bean依赖于的bean name集合
	 *  */
	...
	/**
	 * 返回给定name下注册的（raw原始）单例对象
	 * 查找所有已经实例化的单例bean，和允许提前引用的正在创建的单例bean（从而解决循环引用问题）
	 * 执行流程：
	 * 1. 一级缓存：singletonObjects中直接获取
	 * 2. 二级缓存：earlySingletonObjects中直接获取
	 * 3. 三级缓存：singletonFactories中获取ObjectFactory，执行getObject方法（底层为getEarlyBeanReference方法）
	 * 4. 移除三级缓存，放入二级缓存
	 */
			...
			if (singletonObject == null && allowEarlyReference) {
				...
				try {
					...
					if (singletonObject == null) {
						singletonObject = this.earlySingletonObjects.get(beanName);
						if (singletonObject == null) {
							// 获取ObjectFactory，即doCreateBean中传入的lambda表达式，实际调用的getEarlyBeanReference方法
	...
	/**
	 * 返回给定name下注册的（raw原始）单例对象，
	 * 如果尚未注册则：创建 并 注册 一个新对象
	 * 步骤：
	 * 1. 从单例池singletonObjects获取bean
	 * 2. 从ObjectFactory，执行getObject方法（底层为createBean方法）
	 * 3. 将新建bean放入单例池
	 */
	...
	/**
	 * 销毁给定bean，如果找到对应的可销毁bean实例，则委托给destroyBean方法执行销毁
	 */
	public void destroySingleton(String beanName) {
		// Destroy the corresponding DisposableBean instance.
		// This also triggers the destruction of dependent beans.
		// 销毁对应的DisposableBean实例，也会触发依赖bean的销毁
		...
		// 完成销毁后，从singletonBean集合中移除该bean
	...
	/**
	 * 销毁指定的bean，必须先销毁依赖于该bean的其他bean，不应该抛出任何异常
	 */
	protected void destroyBean(String beanName, @Nullable DisposableBean bean) {
		...
		if (bean != null) {
			try {
				// 执行可销毁bean的销毁，实际为：DisposableBeanAdapter
				bean.destroy();
			}
			catch (Throwable ex) {
