...
final class PostProcessorRegistrationDelegate {
	...
	public static void registerBeanPostProcessors(
		...
		// 从factory的bean-definition中获取所有BPP的bean定义
		String[] postProcessorNames = beanFactory.getBeanNamesForType(BeanPostProcessor.class, true, false);

		// Register BeanPostProcessorChecker that logs a warn message when
		// a bean is created during BeanPostProcessor instantiation, i.e. when
		// a bean is not eligible for getting processed by all BeanPostProcessors.
		// 注册Checker处理器，当在BPP实例化过程中创建bean时（当某个bean不符合被所有BPP处理的条件时），记录一条告警信息
		// 此处bpp总数为factory已有BPP实例+1，即checker也加入进去后的数量
		...
		for (String ppName : postProcessorNames) {
			if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
				...
				if (pp instanceof MergedBeanDefinitionPostProcessor) {
					// MergedBeanDefinitionPostProcessor在处理合并bean-definition时进行回调，属于框架内部处理器
		...
		// 重新添加ApplicationListenerDetector处理器到list尾部（以便能够捕获代理等）
